  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
      <header className="mb-10">
        ...
        <p className="mt-3 text-sm text-muted-foreground">국내 활동 스트릿 댄스 크루 디렉토리</p>
      </header>
        ...
        {crews.map((crew) => (
          ...
          >
            <div className="flex items-baseline justify-between gap-2">
              <h2 className="text-lg font-bold group-hover:text-accent">{crew.name}</h2>
              {crew.foundedYear && (
                <span className="text-xs text-muted-foreground">{crew.foundedYear}</span>
              )}
            </div>
            {crew.koreanName && <p className="text-sm text-muted-foreground">{crew.koreanName}</p>}
